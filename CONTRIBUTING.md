...
Secure AI Gateway is a security-focused portfolio project. Changes should preserve its security invariants and zero-cost required verification path.
...
8. The required project path must remain zero-cost; paid cloud resources are optional/reference-only and must require explicit opt-in.
