...
- GitHub repository branch protection/rulesets are account/repository administration controls and are not enforced by application code.
- Dependency audits identify known published vulnerabilities; they cannot detect unknown vulnerabilities or malicious-but-unflagged packages.
...
3. `make demo` passes on a clean or representative local workstation;
