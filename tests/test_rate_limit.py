...
from app.rate_limit import InMemoryRateLimiter, parse_client_rate_limits
...
def test_parse_client_rate_limits_rejects_invalid_configuration(
    ...
    Requires:
        - configured_limits contains an invalid rate-limit configuration.

    Modifies:
        - Nothing.

    Effects:
        - Verifies malformed or unsafe rate-limit configuration is rejected.

    Inputs:
        - configured_limits: Invalid serialized rate-limit policy.

    Outputs:
        - None. Assertions determine whether parsing fails closed.
    """
...
def test_in_memory_rate_limiter_resets_after_window() -> None:
    ...
    denied = limiter.check("client-a", 2)
    ...
    now[0] += 60.0
    reset = limiter.check("client-a", 2)
...
def test_chat_rate_limit_returns_429(monkeypatch, gateway_api_key) -> None:
    ...
    Modifies:
        ...
        - Process-local gateway rate-limit state.
...
def test_chat_fails_closed_without_client_rate_policy(
    ...
    Modifies:
        - Temporarily removes SAG_CLIENT_RATE_LIMITS.

    Effects:
        - Verifies authenticated chat requests fail closed without rate-limit policy.
